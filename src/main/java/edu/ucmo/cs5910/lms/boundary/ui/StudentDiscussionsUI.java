/*
 * Class: edu.ucmo.cs5910.lms.boundary.ui.StudentDiscussionsUI
 */
...
public class StudentDiscussionsUI {

    @Autowired
    private DiscussionsMgr discussionsMgr;

    @RequestMapping(value = "/userMain/STUDENT/discussions/{classId}", method = RequestMethod.GET)
    public String getAjaxContentJSP(@PathVariable("classId") int argClassId, HttpSession session, Model model) {
        // TODO: replace this logic with logic to build the Student Discussions page
        User userS = (User) session.getAttribute("user");

        model.addAttribute("area", "discussions");
        model.addAttribute("classId", argClassId);
        model.addAttribute("msg", String.format("this is html response from ajax call to /userMain/STUDENT/discussions/%d for user %s in session", argClassId, userS.getLoginName()));
